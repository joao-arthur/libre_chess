...
pub fn movements_of_piece(board: &Board, bounds: &RectU8, pos: &Pos) -> Vec<Pos> {
...
pub fn movements_of_player(board: &Board, bounds: &RectU8, color: &Color) -> HashSet<Pos> {
...
mod tests {
    ...
    fn movements_of_piece_pawn() {
        let board = HashMap::from([piece::of_str("C5", "♙")]);
        let bounds = standard_chess().bounds;
        assert_eq!(movements_of_piece(&board, &bounds, &Pos::of_str("C5")), [Pos::of_str("C6")]);
    }
}
